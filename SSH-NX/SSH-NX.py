...
#markdown
st.markdown('This application is all about tweet sentiment analysis of Covid19 Lockdown.')
#sidebar
st.sidebar.title('Sentiment analysis of Lockdown')
# sidebar markdown 
st.sidebar.markdown("🛫NUST-DARA Hackathon.🛫")
...
st.write(data.query('VADERsentiment==@tweets')[['text']].sample(1).iat[0,0])
st.write(data.query('VADERsentiment==@tweets')[['text']].sample(1).iat[0,0])
st.write(data.query('VADERsentiment==@tweets')[['text']].sample(1).iat[0,0])
